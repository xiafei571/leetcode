...
public class UserResourceLog {
	...
	private static void question2(List<Event> input) {
		...
		Deque<Event> dq = new LinkedList<Event>();
		Collections.sort(input, (a,b) -> a.time - b.time);
		for(Event event : input) {
			while(!dq.isEmpty() && event.time - dq.peekFirst().time > 300) {
				Event old = dq.pollFirst();
				Integer num = resourceMap.get(old.resourceId);
				num--;
				resourceMap.put(old.resourceId, num);
			}
			dq.add(event);
			Integer num = resourceMap.getOrDefault(event.resourceId, 0);
			num++;
			if(num > maxNum) {
				maxNum = num;
				maxResource = event.resourceId;
			}
			resourceMap.put(event.resourceId, num);
		}
		
		System.out.println(maxResource+","+maxNum);
	}
	
	private static void question1(List<Event> input) {
		Map<String, List<Integer>> map = new HashMap<String, List<Integer>>();
		for(Event event : input) {
			List<Integer> list = map.getOrDefault(event.userId, new ArrayList<Integer>());
			int idx = 0;
			while(idx < list.size()) {
				if(list.get(idx) > event.time) {
					break;
				}
				idx++;
			}
			list.add(idx, event.time);
			map.put(event.userId, list);
		}
		for(String key : map.keySet()) {
			List<Integer> list = map.get(key);
			System.out.println(key+":("+list.get(0)+","+list.get(list.size()-1)+")");
		}
	}
